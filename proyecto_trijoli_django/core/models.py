...
class EventConclusion(models.Model):
    evento_realizado = models.ForeignKey(Event, on_delete=models.CASCADE, verbose_name='Evento Realizado')
    descripcion_evento = models.TextField(verbose_name='Descripción del Evento')
    comentarios_patrocinador = models.TextField(verbose_name='Comentarios del Patrocinador', blank=True, null=True)
    comentarios_fundacion = models.TextField(verbose_name='Comentarios de la Fundación', blank=True, null=True)
    articulos_entregados = models.ManyToManyField(Article, verbose_name='Artículos Entregados', related_name='entregados')
    cantidad_entregada = models.PositiveIntegerField(verbose_name='Cantidad Entregada')
    articulos_no_entregados = models.ManyToManyField(Article, verbose_name='Artículos No Entregados', related_name='no_entregados')
    ...
    def __str__(self):
        return f"Final del Evento - {self.evento_realizado}"